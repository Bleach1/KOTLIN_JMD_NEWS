...
public class DiffCallBackUtil extends DiffUtil.Callback {
    ...
    public DiffCallBackUtil(List<BaseBean> mOldDatas, List<BaseBean> mNewDatas) {
     ...
     * DiffCallBackUtil uses this information to detect if the contents of an item has changed.
     * DiffUtil用返回的信息（true false）来检测当前item的内容是否发生了变化
     * DiffCallBackUtil uses this method to check equality instead of {@link Object#equals(Object)}
     * DiffCallBackUtil 用这个方法替代equals方法去检查是否相等。
     * so that you can change its behavior depending on your UI.
     * 所以你可以根据你的UI去改变它的返回值
     * For example, if you are using DiffCallBackUtil with a
